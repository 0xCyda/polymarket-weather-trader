...
CONFIG_SCHEMA = {
    ...
    "punt_price_ceiling":{"env": "SIMMER_WEATHER_PUNT_PRICE_CEILING","default": 0.149, "type": float,
                          "help": "Max price for a punt candidate (14.9¢ default). Sits one tick below CORE's price floor so PUNT and CORE never overlap on the same cent. CORE handles ≥15¢, PUNT handles ≤14.9¢."},
    ...
    "late_price_ceiling":{"env": "SIMMER_WEATHER_LATE_PRICE_CEILING","default": 0.95,  "type": float,
                          "help": "Max entry price for a LATE candidate. Breakeven on good cities is ~0.91 after Simmer fee. Per-city overrides in late_trader.py LATE_CITY_CEILINGS take precedence when tighter."},
    "late_price_floor":  {"env": "SIMMER_WEATHER_LATE_PRICE_FLOOR",  "default": 0.20,  "type": float,
                          "help": "Min entry price for a LATE candidate. Sits just above the PUNT zone (PUNT handles <=0.149) so LATE bridges 0.20-0.95. Lowered from 0.55 (2026-04-26) since the LATE thesis (TWC running max already inside bucket + edge buffer) is independent enough not to need market-agreement validation. Below 0.20, the market is firmly NO and the running-max signal is likely a noisy early-day reading."},
    ...
    "position_pre_peak_breakout_c": {"env": "SIMMER_WEATHER_POSITION_PRE_PEAK_BREAKOUT_C", "default": 1.5, "type": float,
    ...
    "position_projected_exit_buffer_c": {"env": "SIMMER_WEATHER_POSITION_PROJECTED_EXIT_BUFFER_C", "default": 1.0, "type": float,
                          "help": "Projected drift: require projected EOD temp to sit at least this many °C outside the held bucket before force-exiting."},
    ...
    "position_halfhour_min_local_hour": {"env": "SIMMER_WEATHER_POSITION_HALFHOUR_MIN_LOCAL_HOUR", "default": 13, "type": int,
                          "help": "Earliest local hour where adaptive half-hour monitoring can begin if observations already look peak-locked."},
    "position_halfhour_max_local_hour": {"env": "SIMMER_WEATHER_POSITION_HALFHOUR_MAX_LOCAL_HOUR", "default": 19, "type": int,
                          "help": "Latest local hour where adaptive half-hour monitoring is still worth doing for same-day positions."},
    "position_peak_lock_min_minutes": {"env": "SIMMER_WEATHER_POSITION_PEAK_LOCK_MIN_MINUTES", "default": 90.0, "type": float,
                          "help": "Minutes since the last new intraday extreme before the position manager treats the day's running extreme as peak-locked."},
    "position_peak_lock_buffer_c": {"env": "SIMMER_WEATHER_POSITION_PEAK_LOCK_BUFFER_C", "default": 0.2, "type": float,
                          "help": "Small forward temperature buffer used once a same-day city looks peak-locked from observations."},
    ...
    "position_hard_stop_drawdown_frac": {"env": "SIMMER_WEATHER_POSITION_HARD_STOP_DRAWDOWN_FRAC", "default": 0.65, "type": float,
                          "help": "Universal price-only hard stop. 0.65 exits any YES position once live price is down 65% from entry, before weather gates."},
    ...
    "core_low_edge_exact_carveout_enabled": {"env": "SIMMER_WEATHER_CORE_LOW_EDGE_EXACT_CARVEOUT_ENABLED", "default": False, "type": bool,
                          "help": "Enable a narrow CORE carve-out for strong exact buckets in the 35-40¢ band even when edge is below min_edge."},
    ...
    "core_low_edge_exact_carveout_min_price": {"env": "SIMMER_WEATHER_CORE_LOW_EDGE_EXACT_CARVEOUT_MIN_PRICE", "default": 0.35, "type": float,
    ...
    "position_exact_core_market_collapse_floor": {"env": "SIMMER_WEATHER_POSITION_EXACT_CORE_MARKET_COLLAPSE_FLOOR", "default": 0.12, "type": float,
                          "help": "Non-carve exact CORE hard stop: if same-day price collapses below this floor and most entry value is gone, exit even if weather still appears in-bucket. CARVE ignores this before take-profit."},
    "position_exact_core_market_collapse_drop_frac": {"env": "SIMMER_WEATHER_POSITION_EXACT_CORE_MARKET_COLLAPSE_DROP_FRAC", "default": 0.65, "type": float,
                          "help": "Non-carve exact CORE hard stop also requires this drawdown from the best manager-seen price."},
    "position_exact_core_weak_price_floor": {"env": "SIMMER_WEATHER_POSITION_EXACT_CORE_WEAK_PRICE_FLOOR", "default": 0.08, "type": float,
                          "help": "Earlier PM escape hatch for exact CORE trades: if price sinks to this floor and weather support is weak, exit before the universal hard stop has to do all the work."},
...
LOCATION_BIAS_C = {
    # Recalibrated 2026-05-09 from 618 resolved-date forecasts vs Polymarket
    ...
    # Retain cities with meaningful signal depth, generally n>=10 and |delta|>=1.0°C.
    "Chongqing":     3.5,
    "London":        3.2,
    "Beijing":       3.2,
    "Seattle":       2.6,
...
def run_weather_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    for c in top_n:
        ...
        # Per-model breakdown is debug-only — kept off the dashboard summary.
        # The user-facing line is the single Bought/PUNT line below.
        tag = "SIMULATED" if dry_run else "LIVE"
